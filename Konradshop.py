...
LOCAL_VERSION_PATH = os.path.join(os.environ['APPDATA'], 'KonradShop', 'current_version.txt')

def check_security():
    # 1. เช็คว่ารันผ่าน Launcher หรือไม่ (ตรวจสอบ Secret Key)
    # เราจะตั้งกุญแจลับว่า "KonradSecureOpen777"
    if len(sys.argv) < 2 or sys.argv[1] != "KonradSecureOpen777":
        print("Access Denied: Please open via Konrad Launcher.")
        sys.exit()

    # 2. เช็คเวอร์ชันซ้ำว่าตรงกับบน GitHub ไหม (ป้องกันการใช้ไฟล์เก่า)
    try:
        # อ่านเวอร์ชันในเครื่อง
        if not os.path.exists(LOCAL_VERSION_PATH):
            sys.exit()
        with open(LOCAL_VERSION_PATH, "r") as f:
            current_v = f.read().strip()

        # อ่านเวอร์ชันบน GitHub
        res = requests.get(f"{RAW_BASE_URL}/{VERSION_FILE}", timeout=5)
        latest_v = res.text.strip()

        if latest_v > current_v:
            print("Update required!")
            sys.exit()
    except:
        pass # ถ้าเน็ตหลุดแต่มีกุญแจลับ ให้ยอมให้รันได้

# เรียกใช้ฟังก์ชันตรวจสอบทันทีที่เริ่มโปรแกรม
check_security()
...
class MainMenu(ctk.CTk):
    def __init__(self):
        ...
        ctk.CTkButton(self, text="AFKPD(เหม่อกินข้าว)", 
                       command=run_eat, width=280, height=60).pack(pady=20)
        def run_eat():
            target_exe = "Bootfpsv2.exe"
            if os.path.exists(target_exe):
                try:
        ...
        ctk.CTkButton(self, text="OptimizerAppv.1(ไม่โดนแบน)", 
                       command=run_eat, width=280, height=60).pack(pady=20)
